...
In this case, it is not possible to recover, $\tilde{\Xs}_s$, the sectoral
...
$X_i=\sum_{s=1}^{S_0}w_{is}\Xs_{s}$, (ii) aggregate the sectors, or (iii) if the
only controls are those with shift-share structure, and we have data on $\Zs_{s}$,
we can estimate $\tilde{\Xs}_{s}$ by running a sector-level regression of $\Xs_s$
onto $\Zs_s$, and taking the residual. This third option is not currently
...
$X$, this has to be done before using the `reg_ss` and `ivreg_ss` functions.
